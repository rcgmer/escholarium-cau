(function (blink) {
	...
	ModernStyle.prototype = {
		...
		bodyClassName: 'content_type_clase_escholariumrea',
		extraPlugins: ['image2'],
		ckEditorStyles: {
			name: 'escholariumrea',
			styles: [
	...
	blink.theme.styles.escholariumrea = ModernStyle;
...
$(document).ready(function () {
	...
	var src_logo = $('.content_type_clase_escholariumrea').find('.logo_slide').attr('logo_dominio');
	if (typeof(src_logo) != 'undefined' && src_logo && src_logo != '' && src_logo.indexOf('gif1x1.gif') == -1) {
		$('.content_type_clase_escholariumrea').find('.logo-publisher').css('background-image', "url('"+src_logo+"')");
	}
});
